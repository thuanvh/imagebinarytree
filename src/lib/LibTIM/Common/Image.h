...
///Container base for images of generic type T in LibTIM
  ...
  **/
  ...
  public:
    ...
    const TSize &getSizeX() const {
      return size[0];
    }

    const TSize &getSizeY() const {
      return size[1];
    }

    const TSize &getSizeZ() const {
      return size[2];
    }
    ...
    const TSpacing *getSpacing() const {
      return spacing;
    }

    const TSpacing &getSpacingX() const {
      return spacing[0];
    }

    const TSpacing &getSpacingY() const {
      return spacing[1];
    }

    const TSpacing &getSpacingZ() const {
      return spacing[2];
    }

    void setSpacingX(const TSpacing &vx) {
      spacing[0] = vx;
    }

    void setSpacingY(const TSpacing &vy) {
      spacing[1] = vy;
    }

    void setSpacingZ(const TSpacing &vz) {
      spacing[2] = vz;
    }
    ...
    const TOffset &getBufSize() const {
      return dataSize;
    }
    ...
    inline iterator end() {
      return iterator(this, this->data + this->dataSize);
    }

    inline const_iterator end() const {
      return const_iterator(this, this->data + this->dataSize);
    }
    ...
    inline T &operator()(TOffset offset) {
      return data[offset];
    }

    ///Offset read-only version
    inline T operator()(TOffset offset) const {
      return data[offset];
    }
    ...
    int getOffset(TCoord x, TCoord y = 0, TCoord z = 0) {
      return x + y*size[0] + z*size[0]*size[1];
    }

    int getOffset(Point <TCoord> p) {
      return p.x + p.y*size[0] + p.z*size[0]*size[1];
    }
