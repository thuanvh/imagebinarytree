...
namespace LibTIM {
  ...
  public:
    ...
    struct HeaderInfo {
      ...
      static std::string vdim()    {
        return std::string("");
      }

      static std::string pixsize() {
        return std::string("");
      }
  ...
  inline bool CHeaderIO_InrImage::isOfType(std::type_info const & ti) {
    if (parsedHeader_.count("PIXSIZE") == 0 || parsedHeader_.count("TYPE") == 0)
      this->load();
    ...
    */
    ...
    if (parsedHeader_["VDIM"] == "1") {
      if (baseType == "unsigned fixed" && nbBytes == "8 bits" &&  ti == typeid(U8))
        return true;
      else
        if (baseType == "unsigned fixed" && nbBytes == "16 bits" && ti == typeid(U16))
          return true;
        else
          if (baseType == "unsigned fixed" && nbBytes == "32 bits" && ti == typeid(U32))
            return true;

      // else if(baseType=="unsigned fixed" && nbBytes=="64 bits" && ti==typeid(U64)) return true;
          else
            if (baseType == "signed fixed"   && nbBytes == "8 bits"  && ti == typeid(S8))
              return true;
            else
              if (baseType == "signed fixed"   && nbBytes == "16 bits" && ti == typeid(S16))
                return true;
              else
                if (baseType == "signed fixed"   && nbBytes == "32 bits" && ti == typeid(S32))
                  return true;
      ...
      if (parsedHeader_["VDIM"] == "3") {
        if (baseType == "unsigned fixed" && nbBytes == "8 bits" && ti == typeid(RGB))
          return true;
        else
          return false;
      } else
  ...
  inline const std::type_info & CHeaderIO_InrImage::getType() {
    ...
    */
    ...
    if (parsedHeader_["VDIM"] == "1") {
      if (baseType == "unsigned fixed" && nbBytes == "8 bits")
        return typeid(U8);
      else
        if (baseType == "unsigned fixed" && nbBytes == "16 bits")
          return typeid(U16);
        else
          if (baseType == "unsigned fixed" && nbBytes == "32 bits")
            return typeid(U32);

      //else if(baseType=="unsigned fixed" && nbBytes=="64 bits") return typeid(U64);
          else
            if (baseType == "signed fixed"   && nbBytes == "8 bits")
              return typeid(S8);
            else
              if (baseType == "signed fixed"   && nbBytes == "16 bits")
                return typeid(S16);
              else
                if (baseType == "signed fixed"   && nbBytes == "32 bits")
                  return typeid(S32);
  ...
  inline CImageHeader CHeaderIO_InrImage::load() {
    ...
    * in the file.
    ...
    if (parsedHeader_.count("VX") == 1)
      h.spacing[0] = atof(parsedHeader_["VX"].c_str());

    if (parsedHeader_.count("VY") == 1)
      h.spacing[1] = atof(parsedHeader_["VY"].c_str());

    if (parsedHeader_.count("VZ") == 1)
      h.spacing[2] = atof(parsedHeader_["VZ"].c_str());
...
//     else if(ti==typeid(RGB)) headerString << "PIXSIZE=8 bits\nTYPE=unsigned fixed\n";
                ...
                if (ti == typeid(char)) { // find if it is signed or unsigned, and behave appropriately
                  if (std::numeric_limits<char>::is_signed)
                    headerString << "PIXSIZE=8 bits\nTYPE=signed fixed\n";
                  else
                    headerString << "PIXSIZE=8 bits\nTYPE=unsigned fixed\n";
                } else
                  if (ti == typeid(int)) { // find if it is short or long, or something else
                    if (std::numeric_limits<int>::min() == std::numeric_limits<S16>::min() &&
                        std::numeric_limits<int>::max() == std::numeric_limits<S16>::max()) {
                      headerString << "PIXSIZE=16 bits\nTYPE=signed fixed\n";
                    } else
                      if (std::numeric_limits<int>::min() == std::numeric_limits<S32>::min() &&
                          std::numeric_limits<int>::max() == std::numeric_limits<S32>::max()) {
                        headerString << "PIXSIZE=32 bits\nTYPE=signed fixed\n";
                      } else
                        assert(false); // int neither long nor short.
                  } else
