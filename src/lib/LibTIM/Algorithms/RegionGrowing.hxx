  ...
  **/
  ...
  void RegionGrowingCriterion(Image <T> &src, Image <T2> &marker, FlatSE &se, bool observe = false) {
    ...
    while (!stop) {
      ...
      if (bestOffsetList.size() > 1)
        std::cout << iter << " Egalit� entre plusieurs points!!!!\n";
...
///This gives slightly altered results
  ...
  **/
  ...
  template <class T2>
  void seededRegionGrowing0(Image <U8C3> &img, Image <T2> &marker, FlatSE &se, bool observe = false) {
    OrderedQueueDouble <TOffset> oq;

    Image <U8C3> imBorder = img;
    Image <T2> markerBorder = marker;
    ...
    U8C3 maxValueT = std::numeric_limits<U8C3>::max();
    T2 maxValueT2 = std::numeric_limits<T2>::max();
