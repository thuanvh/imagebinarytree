...
///For each marker compute the mean of the points on original image
  ...
  Image <T> computeMarkerMean(Image <T> &src, Image <T2> &marker) {
    ...
    for (tVectorD::iterator it = sumGreyLevel.begin(); it != sumGreyLevel.end(); ++it)
      *it = 0;

    for (tVectorI::iterator it = nbPoints.begin(); it != nbPoints.end(); ++it)
      *it = 0;
...
///For each marker compute the mean of the points on original image
  ...
  Image <T> computeMarkerMeanFast(Image <T> &src, Image <T2> &marker) {
    ...
    for (VectorD::iterator it = sumGreyLevel.begin(); it != sumGreyLevel.end(); it++)
      *it = 0;

    for (VectorI::iterator it = nbPoints.begin(); it != nbPoints.end(); it++)
      *it = 0;
...
///For each marker compute the mean of the points on original image
  ...
  template <class T2>
  Image <U8C3> computeMarkerMean(Image <U8C3> &src, Image <T2> &marker) {
    Image<U8C3> res(src.getSize());
